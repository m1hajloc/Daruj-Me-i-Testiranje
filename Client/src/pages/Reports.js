...
function Reports(){
  ...
  const fetchData = async () => {
      const response = await fetch(`https://localhost:7294/api/Report/GetAllReports`, {
          method: 'GET',
          headers: {
  ...
  const handleCheck = async (report) => {
    const response = await fetch(`https://localhost:7294/api/Report/DeleteReport?id=${report.id}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + Cookies.get('jwt')
  ...
  const handleDelete = async (report) => {
    ...
    const response = await fetch(`https://localhost:7294/api/Product/DeleteProduct?id=${report.product.id}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + Cookies.get('jwt')
                        ...
                        {reports.map((report, index) => (
                            <div key={index} className='report-item'>
                              ...
                              <div className='reports-buttons'>
                                <label id = "okprod" className='button-report' onClick={() => handleCheck(report)}><i class="bi bi-check-lg"></i></label>
                                <label id = "delprod" className='button-report' onClick={() => handleDelete(report)}><i class="bi bi-trash3-fill"></i></label>
                              </div>
                            </div>
                        ))}
                    </div>
