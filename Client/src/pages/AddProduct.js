...
const clearProfilePicture = () => {
  ...
  useEffect(() => {
    const fetchProductTypes = async () => {
      try {
        const response = await axios.get(`https://localhost:7294/api/ProductType/GetAllProductType`);
  ...
  const url='https://localhost:7294/api/Product/CreateProduct';
                  ...
                  <form onSubmit={submit}>
                    ...
                    <div className="register-image-div">
                      ...
                      <MDBInput
                        ...
                        id='description'
                      />
                      <select id = "selectMenu" onChange={(e) => setData({ ...data, productTypeId: e.target.value })}>
                        {productTypes.map(type=>(
                          <option value={type.id}>{type.name}</option>
                        ))}
                      </select>
                    </div>
                    <MDBBtn type="submit" id = "addproduct" submit='true' >Add</MDBBtn>
                  </form>
                </div>
              </MDBCardBody>
            </MDBCard>
