...
function MyReservations({ userId }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`https://localhost:7294/api/Reservation/GetReservationsByCustomerId?id=${userId}`, {
                    headers: {
                ...
                const productData = await Promise.all(
                    reservationsData.map(async (reservation) => {
                        const productResponse = await fetch(`https://localhost:7294/api/Product/GetProductById?id=${reservation.productId}`, {
                            headers: {
                                'Content-Type': 'application/json',
                                'Accept': 'application/json',
                            },
