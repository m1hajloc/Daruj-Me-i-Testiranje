...
function UserReports(){
  ...
  const fetchData = async () => {
      const response = await fetch(`https://localhost:7294/api/UserReport/GetAllUserReports`, {
          method: 'GET',
          headers: {
   ...
   const handleCheck = async (report) => {
    const response = await fetch(`https://localhost:7294/api/UserReport/DeleteUserReport?id=${report.id}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + Cookies.get('jwt')
   ...
   const handleDelete = async (report) => {
    ...
    const response = await fetch(`https://localhost:7294/api/User/DeleteUser?id=${report.reportedUser.id}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + Cookies.get('jwt')
