...
function Product({ product, showButton, userId, refreshProducts, showButton2, products, setProducts, sendBtn }) {
  ...
  function submit(e) {
    ...
    axios.post('https://localhost:7294/api/Reservation/CreateReservation', {
  ...
  function handleDelete() {
    ...
    axios
      .delete(`https://localhost:7294/api/Product/DeleteProduct?id=${productId}`, {
        headers: {
          'accept': '*/*',
        },
      })
...
// ), [showLabel])
  ...
  const url = "https://localhost:7294/api/Product/ChangeState"
  ...
  const handleReportProduct = async () => {
    ...
    const response = await axios.post(
      'https://localhost:7294/api/Report/CreateReport',
      reportData,
      {
        headers: {
  ...
  return (
    <div className='product-card'>
      ...
      <button id={`reserveButton_${product.id}`} style={{ backgroundColor: '#1d1d1d', color: 'white' }} className='report-product-label' onClick={toggleModal}>Report</button>
      <div className="product-content">
        ...
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {showButton && <button id={`reserve1Button_${product.id}`} type="submit" onClick={(e) => submit(e)} className="download-button" style={{ backgroundColor: '#1d1d1d', color: 'white', borderRadius: '8px' }}>Reserve</button>}
          {product && !product.isSent && showButton2 && <button id={`deleteButton_${product.id}`} type="submit" onClick={handleDelete} className="download-button" style={{ backgroundColor: '#1d1d1d', color: 'white', borderRadius: '8px' }}>Delete</button>}
              ...
              <div className="modal-content">
                ...
                <div className="modal-body">
                  <div className='report-product-div'>
                    <textarea id = "textmodal1" className="report-product-input" rows="3" placeholder='Report product...' type='text' required onChange={(e) => setReportProductContent(e.target.value)} />
                  </div>
                </div>
                <div className="modal-footer">
                  <button type="button"  className="btn btn-secondary" onClick={toggleModal}>Close</button>
                  <button type="button" id = "btnmodal1" className="btn btn-primary" onClick={handleReportProduct}>Report</button>
                </div>
              </div>
            </div>
          </div>
